...
def process_one_customer_transcript(df):
    ...
    for _, row in df.iterrows():
        if row.event == "offer received":
            # offer received means a new offer entry should be added
            ...
            # we loop offer list in reversed order
            # see the most recent received but not 
            # finished offer or not expired offer
            # we then add view time to that offer entry
            ...
            # we loop offer list in reversed order
            # see the most recent offer entry which 
            # is not completed, we then add 
            # complete time to this offer entry 
            for i in range(len(res)-1, -1, -1):
            ...
            # if the offer is not completed and 
            # still in valid time range, we add
            # this amount to amount spent in offer entry
